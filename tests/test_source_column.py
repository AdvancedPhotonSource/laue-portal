...
class TestSourceColumnStructure:
    ...
    def test_column_order(self, empty_test_database):
        ...
        assert src_idx == pi_idx + 1, (
            f"Source (idx {src_idx}) should immediately follow Peak Indexing ID (idx {pi_idx})"
        )
...
class TestSourceColumnRowData:
    ...
    def test_wire_recon_row_data(self):
        ...
        from tests.conftest import create_test_catalog, create_test_job, create_test_metadata
...
class TestSourceLinksRendererJS:
    ...
    def test_scan_link_url(self):
        assert "'/scan?scan_id='" in self.js_content or "'/scan?scan_id=' + data.scanNumber" in self.js_content, (
    ...
    def test_mask_recon_commented_out(self):
        ...
        for line in self.navbar_content.splitlines():
            if "Mask Reconstructions" in line:
                ...
                assert stripped.startswith("#"), "The 'Mask Reconstructions' navbar entry should be commented out"
                return
