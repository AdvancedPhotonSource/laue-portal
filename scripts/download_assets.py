...
ZEPHYR_CSS_URL = "https://cdn.jsdelivr.net/npm/bootswatch@5.3.6/dist/zephyr/bootstrap.min.css"
...
# Google Fonts CSS endpoint for Lato (referenced by Zephyr theme via @import)
...
def _download_google_fonts(force=False):
    ...
    Returns the ``@font-face`` CSS text to embed in the Zephyr stylesheet.
    """
...
def _patch_zephyr_css(lato_font_face_css, force=False):
    """
    Download the Zephyr Bootstrap CSS and replace the Google Fonts @import
    with local @font-face declarations.

    *lato_font_face_css* is the ``@font-face`` CSS text generated by
    :func:`_download_google_fonts`.
    """
    dest = os.path.join(ASSETS_DIR, "01-bootstrap-zephyr.min.css")
    if os.path.exists(dest) and not force:
        print(f"  [skip] {os.path.relpath(dest, PROJECT_ROOT)} (already exists)")
        return

    print("\nDownloading Zephyr Bootstrap CSS...")
    css = _download_text(ZEPHYR_CSS_URL)
    ...
    if n == 0:
        print("  [warn] No Google Fonts @import found in Zephyr CSS to replace.")
...
def main():
    ...
    # 2. Lato font files from Google Fonts (must run before Zephyr CSS patch)
    print("\n--- Lato Font (Google Fonts) ---")
    lato_font_face_css = _download_google_fonts(force=force)

    # 3. Zephyr Bootstrap CSS (with Google Fonts @import replaced by local @font-face)
    print("\n--- Zephyr Bootstrap CSS ---")
    _patch_zephyr_css(lato_font_face_css, force=force)
