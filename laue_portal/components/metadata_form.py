                    ...
                    [
                        _stack(
                            [
                                ...
                                html.Div([_field("Time Epoch", "time_epoch", size="lg")], style={"display": "none"}),
                    ...
                    [
                        ...
                        _stack(
                            [
                                ...
                                html.Div(
                                    [_field("Time Epoch", "scanEnd_time_epoch", size="lg")], style={"display": "none"}
                                ),
...
def _make_positioner_fields(scan_data, i, pos_num, read_only):
    ...
    if _should_display(pv_val):
        fields.append(
            _field(
                f"Positioner {pos_num} PV",
                {"type": pv_attr, "index": i},
                size="md",
    ...
    if _should_display(getattr(scan_data, name_attr)):
        fields.append(
            _field(
                f"Positioner {pos_num}",
                {"type": name_attr, "index": i},
                size="md",
    ...
    if _should_display(getattr(scan_data, ar_attr)):
        fields.append(
            _field(
                f"Positioner {pos_num} ar",
                {"type": ar_attr, "index": i},
                size="md",
    ...
    if _should_display(getattr(scan_data, mode_attr)):
        fields.append(
            _field(
                f"Positioner {pos_num} mode",
                {"type": mode_attr, "index": i},
                size="md",
...
def _make_detector_fields(scan_data, i, det_num, read_only):
    ...
    if _should_display(getattr(scan_data, val_attr)):
        fields.append(
            _field(
                f"Detector Trig {det_num} VAL",
                {"type": val_attr, "index": i},
                size="sm",
...
def set_metadata_form_props(metadata, scans=None, read_only=True):
    ...
    set_props(
        "scanEnd_source_ringCurrent_unit", {"value": metadata.scanEnd_source_ringCurrent_unit, "readonly": read_only}
    )
...
def set_scan_accordions(scan_rows, read_only=True):
    ...
    scan_accordions = [make_scan_accordion(i, scan_row, read_only) for i, scan_row in enumerate(scan_rows)]
