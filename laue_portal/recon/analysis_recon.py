...
def read_indices(idx_file, grain_file=None, grain_idx=0):
    ...
    indices = np.round(np.array(list(zip(df.iloc[:, 1], df.iloc[:, 0], strict=False)))).astype(int)
...
def define_vars(config_dict, indices_selection="CALIB_3_X1800"):
    ...
    file_vars = ind_file, dat_file, pos_file, sig_file, scl_file, ene_file, dep_file, lau_file
...
def run_recon(config_dict, indices_selection="CALIB_3_X1800", debug=False):
    ...
    ind_file, dat_file, pos_file, sig_file, scl_file, ene_file, dep_file, lau_file = file_vars
    ...
    # Load results
    if (not resolve_data) and all([Path(f).exists() for f in file_vars[2:]]):
...
def run_analysis(config_dict):
    ...
    process = processes[0]
    if process == 1:
        time_vals = run_recon(config_dict, indices_selection="CALIB_3_X1800")  # noqa: F841

    elif process == 2:
        idx_file = "laue_portal/recon/Si1_XY.txt"
        time_vals = run_recon(config_dict, indices_selection=idx_file)  # noqa: F841
