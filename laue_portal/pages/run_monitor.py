...
from laue_portal.processing.redis_utils import STATUS_MAPPING, STATUS_REVERSE_MAPPING, cancel_batch_job, move_batch_to_front
...
layout = html.Div([
        ...
        ], id="move-front-confirm-modal", is_open=False, centered=True),
...
def update_button_states(selected_rows):
    ...
    return (
        not has_cancellable, disabled_style if not has_cancellable else enabled_style,  # Stop
        not has_queued, disabled_style if not has_queued else enabled_style,  # Move to Front
    )
...
@dash.callback(
    ...
    running=[
        ...
        (Output("stop-confirm-yes-btn", "children"),
         [dbc.Spinner(size="sm", spinner_class_name="me-2"), "Cancelling..."], "Cancel Jobs"),
...
def open_move_front_confirmation(n_clicks, selected_rows):
    ...
    movable = [
        row for row in selected_rows
        if row.get('status') == STATUS_REVERSE_MAPPING["Queued"]
    ]
...
@dash.callback(
    ...
    running=[
        ...
        (Output("move-front-confirm-yes-btn", "children"),
         [dbc.Spinner(size="sm", spinner_class_name="me-2"), "Moving..."], "Move to Front"),
