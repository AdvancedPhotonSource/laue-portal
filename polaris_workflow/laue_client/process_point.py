...
class LaueClient(GladierBaseClient):
    gladier_tools = [UplinkTransfer, QSubLaunch, DownlinkTransfer, DownlinkIndex]
...
if __name__ == "__main__":
    ...
    flow_input = {
        "input": {
            ...
            "downlink_destination_path": os.path.join(
                conf["clutch"]["staging"], "polaris_results", scan_name, "recons", point_folder
            ),
            "downlink_index_source_path": os.path.join(conf["eagle_34ide"]["staging"], index_folder, point_folder),
            "downlink_index_destination_path": os.path.join(
                conf["clutch"]["staging"], "polaris_results", scan_name, "indexes", point_folder
            ),
        }
    }
